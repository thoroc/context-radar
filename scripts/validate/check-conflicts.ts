 ...
 * resolves to neither an id nor a `tool` display name (case-insensitive), so a
 * typo cannot silently drop a conflict edge. Returns human-readable errors,
...
export const checkConflicts = (tools: Tool[]): string[] => {
  ...
  for (const tool of tools) {
    for (const ref of tool.conflict.projects) {
      if (ids.has(ref) || byName.has(ref.toLowerCase())) continue;
